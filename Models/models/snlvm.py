...
from Models.layers import GeneratorGRU, MaskedDense
from Models.losses import vMF_kldiv_loss, poisson_loglike_loss, mse_loss
...
class SNLVM(ModelLoader, tf.keras.Model):
    ...
    def __init__(self, **kwargs: Dict[str, Any]):
        ...
        self.behavior_loglike_loss = mse_loss()
        ...
        self.encoded_var_trainable: bool = ArgsParser.get_or_default_and_remove(
            encoder_args, 'var_trainable', False)
        ...
        self.dense_logvar = tf.keras.layers.Dense(
            1, activation=tf.nn.softplus, name="DenseLogVar", **layers['dense_logvar'])
    ...
    @staticmethod
    def load(filename) -> SNLVM:
        return ModelLoader.load(filename, SNLVM)
    ...
    def call(self, inputs, training: bool = True):
        ...
        log_f, b, z = self.decode(g0, inputs, training=training)
        return log_f, b, (g0, mean, logvar), z
    ...
    def decode(self, g0, inputs, training: bool = True):
        ...
        # Behaviour
        b = self.behavioural_dense(z, training=training)
    ...
    def encode(self, inputs, training: bool = True):
        ...
        mean = mean / tf.sqrt(tf.reduce_sum(tf.square(mean), axis=1, keepdims=True))

        if self.encoded_var_trainable:
            logvar = self.dense_logvar(dropped_encoded, training=training) + 1
            logvar = tf.reshape(logvar, [-1])
        else:
            logvar = self.prior_variance

        p_g0 = tfp.distributions.PowerSpherical(mean, logvar)
        g0 = p_g0.sample()

        return g0, mean, logvar

    def compile(self, optimizer, loss_weights, *args, **kwargs):
        super(SNLVM, self).compile(
            loss=[
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            log_f, b, g, _ = self(x, training=True)
    ...
    def test_step(self, data):
        ...
        x, y, sample_weight = tf.keras.utils.unpack_x_y_sample_weight(data)
        # Run forward pass.
        log_f, b, g, _ = self(x, training=False)
