...
from Models.layers import GaussianSampling, GeneratorGRU, MaskedDense
...
class NFLVM(ModelLoader, tf.keras.Model):
    ...
    def __init__(self, **kwargs: Dict[str, Any]):
        ...
        self.relevant_dense_mean = tf.keras.layers.Dense(
            self.initial_condition_dim, name="RelevantDenseMean", **layers['relevant_dense_mean'])
        self.relevant_dense_logvar = tf.keras.layers.Dense(
            self.initial_condition_dim, name="RelevantDenseLogVar", **layers['relevant_dense_logvar'])
        # Irrelevant
        self.irrelevant_dense_mean = tf.keras.layers.Dense(
            self.initial_condition_dim, name="IrrelevantDenseMean", **layers['irrelevant_dense_mean'])
        self.irrelevant_dense_logvar = tf.keras.layers.Dense(
            self.initial_condition_dim, name="IrrelevantDenseLogVar", **layers['irrelevant_dense_logvar'])
        ...
        if self.decoder_dim != self.initial_condition_dim:
            self.relevant_dense_pre_decoder = tf.keras.layers.Dense(
                self.initial_condition_dim, name="RelevantDensePreDecoder", **layers['relevant_dense_pre_decoder'])
        ...
        if self.relevant_decoder_original_cell:
            relevant_decoder_cell = GeneratorGRU(
                self.decoder_dim, **relevant_decoder_args)
            ...
            self.relevant_decoder = tf.keras.layers.GRU(
                self.decoder_dim, return_sequences=True, time_major=False, name='RelevantDecoderGRU', **relevant_decoder_args)
        # Irrelevant
        if self.decoder_dim != self.initial_condition_dim:
            self.irrelevant_dense_pre_decoder = tf.keras.layers.Dense(
                self.decoder_dim, name="IrrelevantDensePreDecoder", **layers['irrelevant_dense_pre_decoder'])
        ...
        if self.irrelevant_decoder_original_cell:
            irrelevant_decoder_cell = GeneratorGRU(
                self.decoder_dim, **irrelevant_decoder_args)
            ...
            self.irrelevant_decoder = tf.keras.layers.GRU(
                self.decoder_dim, return_sequences=True, time_major=False, name='IrrelevantDecoderGRU', **irrelevant_decoder_args)

        # DIMENSIONALITY REDUCTION
        self.rel_factors_dense = tf.keras.layers.Dense(
            self.factors, use_bias=False, name="RelevantFactorsDense", **layers['rel_factors_dense'])
        self.irr_factors_dense = tf.keras.layers.Dense(
            self.factors, use_bias=False, name="IrrelevantFactorsDense", **layers['irr_factors_dense'])
    ...
    @staticmethod
    def load(filename) -> NFLVM:
        return ModelLoader.load(filename, NFLVM)
    ...
    def call(self, inputs, training: bool = True, test_sample_mode: str='mean'):
        ...
        log_f, z_r = self.decode(
            g0_r, g0_i, inputs, training=training)
        return log_f, (g0_r, mean_r, logvar_r), (g0_i, mean_i, logvar_i), z_r
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            log_f, g_r, g_i, _ = self(x, training=True)

            neural_loglike_loss         = self.compiled_loss._losses[0](log_f,x)
            relevant_kldiv_loss         = self.compiled_loss._losses[1](g_r)
            irrelevant_kldiv_loss       = self.compiled_loss._losses[1](g_i)
            independence_loss           = self.compiled_loss._losses[2](g_r,g_i)

            loss =  self.loss_weights[0] * neural_loglike_loss + \
                    self.loss_weights[1] * relevant_kldiv_loss + \
                    self.loss_weights[1] * irrelevant_kldiv_loss + \
                    self.loss_weights[3] * independence_loss
        ...
        core_logs = {
            'loss_PNLL': self.tracker_loss_neural_loglike.result() / self.tracker_loss_count.result(),
            'loss_KLD_r': self.tracker_loss_relevant_kldiv.result() / self.tracker_loss_count.result(),
            'loss_KLD_i': self.tracker_loss_irrelevant_kldiv.result() / self.tracker_loss_count.result(),
    ...
    def test_step(self, data):
        ...
        x, y, sample_weight = tf.keras.utils.unpack_x_y_sample_weight(data)
        # Run forward pass.
        log_f, g_r, g_i, _ = self(x, training=False)
