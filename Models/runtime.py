...
from Models.utils.args_parser import ArgsParser
...
from Models import TNDM, LFADS, SNLVM, NFLVM
from Models.utils import AdaptiveWeights, logger, CustomEncoder, LearningRateStopping
...
class Runtime(object):
    ...
    def train(
        ...
        loss_weights: list,
        modeldir: str,
        val_dataset: Optional[Tuple[tf.Tensor, tf.Tensor]] = None,
        batch_size: Optional[int] = None,
        logdir: Optional[str] = None,
        adaptive_lr: Optional[Union[dict, tf.keras.callbacks.Callback]] = None,
        layers_settings: Dict[str, Any] = {},
        ...
        elif model_type == ModelType.NFLVM:
            model_settings.update(
                neural_dim=x.shape[-1],
            )
            model = NFLVM(
        ...
        elif model_type == ModelType.SNLVM:
            model_settings.update(
                neural_dim=x.shape[-1],
                behaviour_dim=y.shape[-1],
            )
        ...
        callbacks.append(
            tf.keras.callbacks.EarlyStopping(
                monitor='val_loss_PNLL', min_delta=1, patience=100, verbose=1,
    ...
    def train_from_file(settings_path: str):
        ...
        optimizer, loss_weights, adaptive_lr, epochs, batch_size, terminating_lr = runtime_settings
    ...
    def evaluate_performance(model: tf.keras.Model, neural: tf.Tensor, behaviour: tf.Tensor, \
        ...
        if isinstance(model, TNDM):
            ...
            z = z.numpy()
        elif isinstance(model, NFLVM):
            log_f, _, z = model(neural, training=False)
            z = z.numpy()
        elif isinstance(model, SNLVM):
            log_f, _, z = model(neural, training=False)
            z = z.numpy()
        else:
